...
umidita0 = [random.randint(5, 45) for _ in range(num_rows)]
umidita1 = [umidita0[i] + random.randint(-4, 4) for i in range(num_rows)]
umidita2 = [umidita0[i] + random.randint(-4, 4) for i in range(num_rows)]
umidita3 = [umidita0[i] + random.randint(-4, 4) for i in range(num_rows)]
umidita4 = [umidita0[i] + random.randint(-4, 4) for i in range(num_rows)]
umidita5 = [umidita0[i] + random.randint(-4, 4) for i in range(num_rows)]
umidita6 = [umidita0[i] + random.randint(-4, 4) for i in range(num_rows)]
umidita7 = [umidita0[i] + random.randint(-4, 4) for i in range(num_rows)]
umidita8 = [umidita0[i] + random.randint(-4, 4) for i in range(num_rows)]
umidita9 = [umidita0[i] + random.randint(-4, 4) for i in range(num_rows)]

latitudine = [random.uniform(-90, 90) for _ in range(num_rows)]

longitudine = [random.uniform(-180, 180) for _ in range(num_rows)]

durata = [max(0, 100 - (3 * sm)) * 10 for sm in umidita0]  #durata in secondi
...
class RNNDataset(Dataset):
    def __init__(self, dataframe):
        self.data = dataframe[
            ['Umidità0 (%)','Umidità1 (%)','Umidità2 (%)','Umidità3 (%)',
             'Umidità4 (%)','Umidità5 (%)','Umidità6 (%)','Umidità7 (%)',
             'Umidità8 (%)','Umidità9 (%)','Latitudine', 'Longitudine',
             'Day sin', 'Day cos', 'Year sin', 'Year cos',
             'Durata (s)']].values.astype(np.float32)
...
sample_input = torch.tensor(
    [[6, 8, 3, 5, 8, 5, 2, 9, 7, 10, -34.873877, -178.553346, -0.690049, -0.723762, 0.992370, -0.123297,
      820]])  # Esempio di input con umidità del 50% e segnali sinusoidali casuali

"""
  Umidità0 (%)  Umidità1 (%)  Umidità2 (%)  Umidità3 (%)  Umidità4 (%)  
             6             8             3             5             8   

  Umidità5 (%)  Umidità6 (%)  Umidità7 (%)  Umidità8 (%)  Umidità9 (%)  
             5             2             9             7            10   

  Latitudine  Longitudine    Day sin   Day cos    
  -34.873877  -178.553346  -0.690049 -0.723762   

  Year sin  Year cos  Durata (s)  
  0.992370 -0.123297         820 """
