...
class PrevisioneMeteo:
    ...
    def get_info_all(self):                 # meteo realztivo successivi giorni e ore
        if self.data is not None:
            time = self.data["hourly"]["time"]
            temp = self.data["hourly"]["temperature_2m"]
            wind = self.data["hourly"]["wind_speed_10m"]
            rain = self.data["hourly"]["precipitation"]
            cloud = self.data["hourly"]["cloudcover"]
            weather = self.data["hourly"]["weathercode"]
            previsioni = { "previsioni" : [{"hour":time[i], "temp" : temp[i], "wind" : wind[i], "rain" : rain[i], "cloud": cloud[i], "weather" : self.weather_dict[weather[i]] } for i in range(len(time))] }
            return previsioni
        else:
            print(f"Non è possibile ottenere le informazioni meteo")
            return None
